...
def get_musdb_folds(root_path, version="HQ", guitID=None):
    ...
    if 'AthenuitarSep' in root_path or 'Fake' in root_path:
        ...
        if version == "cross-val":
        ...
        elif version == "HQ" or version == "HQ-half" or version == "HQ-3quarters" or version == "HQ-quarter" or version == "HQ-eighth"  or version == "HQ-sixteenth":
            ...
            train_list_comp_half = np.random.choice(train_val_list_comp, 58, replace=False)
            train_list_solo_half = np.random.choice(train_val_list_solo, 58, replace=False)
            train_list_comp_3quarters = np.random.choice(train_val_list_comp, 87, replace=False)
            train_list_solo_3quarters = np.random.choice(train_val_list_solo, 87, replace=False)
            train_list_comp_quarter = np.random.choice(train_val_list_comp, 29, replace=False)
            train_list_solo_quarter = np.random.choice(train_val_list_solo, 29, replace=False)
            train_list_comp_eighth = np.random.choice(train_val_list_comp, 14, replace=False)
            train_list_solo_eighth = np.random.choice(train_val_list_solo, 14, replace=False)
            train_list_comp_sixteenth = np.random.choice(train_val_list_comp, 7, replace=False)
            train_list_solo_sixteenth = np.random.choice(train_val_list_solo, 7, replace=False)
            ...
            if version == "HQ-half":
                train_list = np.append(train_list_solo_half, train_list_comp_half)
            if version == "HQ-3quarters":
                train_list = np.append(train_list_solo_3quarters, train_list_comp_3quarters)
            if version == "HQ-quarter":
                train_list = np.append(train_list_solo_quarter, train_list_comp_quarter)
            if version == "HQ-eighth":
                train_list = np.append(train_list_solo_eighth, train_list_comp_eighth)
            if version == "HQ-sixteenth":
                train_list = np.append(train_list_solo_sixteenth, train_list_comp_sixteenth)
            ...
            train_val_list_other = np.array([track for track in train_val_list if not '_solo' in track['mix'] and not '_comp' in track['mix']])
            print('train_val_list_comp', len(train_val_list_comp))
            print('train_val_list_solo', len(train_val_list_solo))

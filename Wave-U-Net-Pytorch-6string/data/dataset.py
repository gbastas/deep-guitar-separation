...
class SeparationDataset(Dataset):
    def __init__(self, dataset, partition, instruments, sr, channels, shapes, random_hops, hdf_dir, audio_transform=None, in_memory=False, features=None):
        '''
        ...
        # PREPARE HDF FILE
        print('[dataset.py] PREPARE HDF FILE')
        ...
        if not os.path.exists(self.hdf_dir):
            ...
            with h5py.File(self.hdf_dir, "w") as f:
                ...
                print("Adding audio files to dataset "+partition+" (preprocessing)...")
                for idx, example in enumerate(tqdm(dataset[partition])):
                    ...
                    # ### __gbastas__, sometimes one extra sample ###
                    ...
                    # print(source_audios.shape[1], mix_audio.shape[1])
        ...
        # print('loading dataset:', partition)
        try:
            self.length = self.start_pos[-1]
        except Exception as e:
            print('[gb] May need to delete the corresponding hdf_file. Problem with partition', partition)
            print(e)
            # exit(0)
    ...
    def __getitem__(self, index):
        ...
        if hasattr(self, "audio_transform") and self.audio_transform is not None:
            audio, targets = self.audio_transform(audio, targets)       # gb: here targets gets cropped in length



        if self.features=='stft':
            audio, targets = librosa.stft(audio), librosa.stft(targets)

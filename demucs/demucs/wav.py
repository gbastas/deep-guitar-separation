...
def get_musdb_wav_datasets(args):
    ...
    train_val_list = [name for name, meta in metadata.items()] # __gabastas__

    valid_tracks = get_val_names(train_val_list)
    ...
    print('metadata_train', len([name for name, meta in metadata.items() if name not in valid_tracks]))
...
def get_val_names(train_val_list): #__gbastas__

    train_val_list_comp = np.array([track for track in train_val_list if 'comp' in track])
    train_val_list_solo = np.array([track for track in train_val_list if 'solo' in track])
    train_val_list_other = np.array([track for track in train_val_list if not 'solo' in track and not 'comp' in track]) # change4d
    ...
    train_list = np.append(train_list, train_val_list_other)

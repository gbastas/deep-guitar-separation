...
class Solver(object):
    def __init__(self, loaders, model, optimizer, args):
        ...
        # __gbastas__
        # log_dir = os.path.join(xp.folder,'tensorboard_logs')
        # log_dir = os.path.join('/home/gbastas/journal_string_separation/demucs/tensorboard_logs', str(xp.sig)) 
        log_dir = os.path.join('../tensorboard_logs', str(xp.sig)) 
        ...
        for epoch in range(len(self.history), self.args.epochs):
            ...
            if should_eval or is_last:
                ...
                with states.swap_state(self.model, state):
                    with torch.no_grad():
                        metrics['test'] = evaluate(self, compute_sdr=compute_sdr) # evaluate.py
                        # print("metrics['test']", metrics['test'])
                formatted = self._format_test(metrics['test'])
                logger.info(bold(f"Test Summary | Epoch {epoch + 1} | {_summary(formatted)}"))
                self.writer.add_scalar('test_sdr',metrics['test']['sdr'], epoch)
                self.writer.add_scalar('test_sir',metrics['test']['sir'], epoch)
                self.writer.add_scalar('test_nsdr',metrics['test']['nsdr'], epoch)

...
class TabCNN:
    ...
    def __init__(self, 
        ...
        # if test:
        #     # print('AAAAAAA')
        #     # args.save_folder = args.save_path
        #     directories = [d for d in os.listdir(self.save_path) if os.path.isdir(os.path.join(self.save_path, d))]
        #     directories = [d for d in directories if d.startswith(self.spec_repr)]
        #     sorted_directories = sorted(directories, key=lambda d: d.split()[-1])
        #     latest_directory = sorted_directories[-1]
        #     self.save_folder = os.path.join(self.save_path, latest_directory) + '/'
        #     print("Latest directory:", latest_directory)
        #     print("Latest directory path:", self.save_folder)
        # else:
        if test:
            self.save_folder = self.save_path + args.saved_exp + '/'
        else:
            self.save_folder = self.save_path + self.spec_repr + " " + datetime.datetime.now().strftime("%Y-%m-%d %H%M%S") + "/"
            # self.save_folder = self.save_path + "c 2023-10-27 143732/"
    ...
    def partition_data(self, data_split):
        ...
        if self.partition_mode == 'senvaityte':
            with open('../data/GuitarSet/NMFtestSet.csv', newline='') as csvfile:
    ...
    def train(self):
        self.model.fit_generator(generator=self.training_generator,
                    ...
                    use_multiprocessing=False, # __gbastas__ to avoid error
                    workers=9)
    ...
    def load_weights(self):
        self.model.load_weights(tabcnn.split_folder + "weights.h5")
    ...
    def load_predictions(self):
        data = np.load(self.split_folder + "predictions.npz")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default="../data/spec_repr_mic/", help='spec_repr_mix OR spec_repr_mic')
    args = parser.parse_args()

    tabcnn = TabCNN(data_path=args.data_path, test=args.test, partition_mode=args.partition_mode)

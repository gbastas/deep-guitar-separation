...
def set_seed(seed=42):
    ...
    torch.backends.cudnn.benchmark = False  # Ensure no optimizations for varying input sizes

    # Set the environment for Python hash
...
class CustomDataset(Dataset):
    def __init__(self, data_list, mode, input_feature_type, n_channels):
    ...
    def __getitem__(self, index):
        ...
        input_channels = input_features.shape[0] # __gbasta__
        frame_len = input_features.shape[1]
...
def F0_pad_collate(batch):
    ...
    for batch_n in range(batch_size):
        ...
        # padded_input_features = np.pad(input_features[batch_n], [(0, frame_padlen), (0, 0)], 'constant')
        padded_input_features = np.pad(input_features[batch_n], [(0, 0), (0, frame_padlen), (0, 0)], 'constant')  # Changed
    ...
    print('input features shape:', padded_input_features_out.shape) 
...
def tab_pad_collate(batch):
    ...
    # padded_input_features_out = padded_input_features_out.squeeze(1)  # Remove the extra dimension __gbastas__
    # print('aaaaaaaaa', padded_input_features_out.shape) 
...
def train(mode, input_feature_type, encoder_type, use_custom_decimation_func, use_conv_stack, use_galoss, test_num, train_data_list, valid_data_list, tensorboard_dir, model_dir, epoch, lr,  d_model, encoder_heads, encoder_layers, n_cores, device, n_bins, hop_length, sr, feat_mode):
    ...
    if feat_mode=='standard':
        n_channels=1
        print('AAAAA')
    else:
        n_channels=7

    train_dataset = CustomDataset(train_data_list, mode, input_feature_type, n_channels)
    valid_dataset = CustomDataset(valid_data_list, mode, input_feature_type, n_channels)
    ...
    model = TabEstimator(mode, encoder_type, use_custom_decimation_func, use_conv_stack, n_bins, hop_length, sr, n_channels=n_channels, encoder_heads=encoder_heads, encoder_layers=encoder_layers)
    ...
    def _update(engine, batch):
        ...
        frame_pred, note_pred, olens = model(padded_input_features.float(), frame_len, note_len, bpm) # padded_input_features.shape e.g. [32, 7, 608, 192]
    ...
    def _inference(engine, batch):
        ...
        with torch.no_grad():
            ...
            frame_pred, note_pred, olens = model(padded_input_features.float(), frame_len, note_len, bpm)
            encoder_self_attn_map = model.encoder.encoders._modules['0']._modules['self_attn'].attn
            if mode == "F0":
...
def main(mode, input_feature_type, encoder_type, use_custom_decimation_func, use_conv_stack, use_galoss, train_ratio, note_resolution, epoch, lr, seed_, d_model, encoder_heads, encoder_layers, n_cores, cqt_n_bins, hop_length, sr, partition_mode, npz_path, feat_mode):

    data_path = os.path.join("data", npz_path, feat_mode, "split", "*.npz")
    # data_path = os.path.join("data", npz_path, '7-pred', "split", "*.npz")
    ...
    # __gbastas__
    suffix = npz_path.split('_',1)[-1].split('-',1)[-1]
    suffix = suffix + '_' + feat_mode

    now = datetime.datetime.now()
    tensorboard_dir = os.path.join("tensorboard", "{0:%Y%m%d%H%M}".format(now))
    # model_dir = os.path.join("model", "{0:%Y%m%d%H%M}_".format(now))
    model_dir = os.path.join("model", "{0:%Y%m%d%H%M}_senv_".format(now) + suffix)  # __gbastas__
    ...
    if torch.cuda.is_available():
        ...
        if partition_mode == 'senvaityte':
            ...
            dev_data_list = [datapath for datapath in data_list if '_'.join(os.path.split(datapath)[1].split('_')[:2]) not in testfiles] # NOTE: tesr if len(dav_data_list)==len(data_list), if yes try like predict.py
            if len(dev_data_list)==len(data_list):
                print('No testset was ceated!')
                exit(0)
            else:
                print('TrainValset length:', len(dev_data_list))
                print('Testset length:', len(data_list) - len(dev_data_list))
            random.shuffle(dev_data_list)
            train_data_list = dev_data_list[:int(round(len(dev_data_list) * train_ratio))]
            valid_data_list = dev_data_list[int(round(len(dev_data_list) * train_ratio)):]
            tensorboard_dir = os.path.join(
                "tensorboard", "{0:%Y%m%d%H%M}".format(now), f"test")
            # model_dir = os.path.join("model", "{0:%Y%m%d%H%M}".format(now), f"testNo00")
            model_dir = os.path.join(model_dir, f"testNo00") # __gbastas__
            train(mode, input_feature_type, encoder_type, use_custom_decimation_func, use_conv_stack, use_galoss, 0, train_data_list, valid_data_list, tensorboard_dir, model_dir,
                  epoch, lr, d_model, encoder_heads, encoder_layers, n_cores, device, n_bins, hop_length, sr, feat_mode)
        ...
        #####################################################################################################################################
            ...
            for test_num in range(6):
                ...
                train_data_list = dev_data_list[:int(round(len(dev_data_list) * train_ratio))]
                valid_data_list = dev_data_list[int(round(len(dev_data_list) * train_ratio)):]
                tensorboard_dir = os.path.join("tensorboard", "{0:%Y%m%d%H%M}".format(now), f"testNo0{test_num}")
                # model_dir = os.path.join("model", "{0:%Y%m%d%H%M}".format(now), f"testNo0{test_num}")
                model_dir = os.path.join(model_dir, f"testNo00") # __gbastas__
...
if __name__ == "__main__":
    ...
    main(mode, input_feature_type, encoder_type, use_custom_decimation_func, use_conv_stack, use_galoss, train_ratio, note_resolution, epoch, lr, seed_, d_model, encoder_heads,
         encoder_layers, n_cores, cqt_n_bins, hop_length, sr, partition_mode, npz_path, feat_mode)
